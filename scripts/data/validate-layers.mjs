...
const validateRecord = (layerId, record) => {
  ...
  assert(
    ...
    `${layerId}:${record.id} latitude out of range`
  );
  assert(
    record.longitude >= -180 && record.longitude <= 180,
    `${layerId}:${record.id} longitude out of range`
  );
...
const validateLayer = (layerId, layer) => {
  ...
  assert(
    typeof layer.sourceUrl === "string" && layer.sourceUrl.startsWith("http"),
    `${layerId}: sourceUrl invalid`
  );
  assert(typeof layer.year === "string" && layer.year.length > 0, `${layerId}: year missing`);
  assert(
    typeof layer.updated === "string" && layer.updated.length > 0,
    `${layerId}: updated missing`
  );
